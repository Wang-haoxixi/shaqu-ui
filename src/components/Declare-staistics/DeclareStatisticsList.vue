<template>
    <el-card class="main-container">
      ...
      <h3 class="top-titles">{{pageData.title}}</h3>
      <div class="chose-tab">
...
export default {
  ...
  data() {
      return {
        typeList:{
            wfDeclareStaistics:'getDeclareList',
            yfDeclareStaistics:'getYFDeclareList',
            hmDeclareStaistics:'getHMDeclareList',
            czDeclareStaistics:'getJYDeclareList',
        },
        searchForm:{
          townStreetId:'',
          declareYear:''
  ...
  methods:{
    lodaData(){
      var self = this;
      self.loading = true;
      let fun=this.typeList[this.pageData.typeName];
    ...
    changeType(e){
      ...
      if(e==='2'){
        ...
        this.searchForm.declareYear = (new Date()).getFullYear()-1+'';
      }else {
    ...
    //列表查看详情
    detailRow(index) {
      let obj = {declareYear: this.dataList[index].declareYear};
      //如果是镇街,需要传镇街id
      if(this.type==='2'){
          obj.townStreetName = this.dataList[index].streetName;
      }
      obj.typeName=this.pageData.typeName,
        obj.title=this.pageData.title,
      this.$router.push({
